...
class BookingRepositoryTest {
    ...
    void findAllByItemIdAndStatusTest() {
        ...
        List<Booking> booking = bookingRepository.findAllByItemIdAndStatus(1L, BookingStatus.APPROVED, Pageable.unpaged());
        assertEquals(booking.size(),2);
    ...
    void findAllByBookerIdAndStartBeforeAndEndAfterTest() {
        ...
        List<Booking> booking = bookingRepository.findAllByBookerIdAndStatus(2L, BookingStatus.WAITING, Pageable.unpaged());
        assertEquals(booking.size(), 1);
        assertTrue(booking.get(0).getStart().isAfter(LocalDateTime.now()));

        List<Booking> bookingPast = bookingRepository.findAllByBookerIdAndStartBeforeAndEndAfter(2L, LocalDateTime.now(), Pageable.unpaged());
        assertEquals(bookingPast.size(), 1);
        assertTrue(bookingPast.get(0).getEnd().isAfter(LocalDateTime.now()));

        List<Booking> bookingRejected = bookingRepository.findAllByItemIdAndStatus(2L, BookingStatus.REJECTED, Pageable.unpaged());
    ...
    public void testFindBookingOwnerAllAndCurrentAndFutureAndPastTest() {
        ...
        List<Booking> booking = bookingRepository.findAllByOwnerIdAndEndBefore(1L,LocalDateTime.now(), Pageable.unpaged());
        assertEquals(booking.size(),1);
        assertTrue(booking.get(0).getStart().isBefore(LocalDateTime.now()));

        List<Booking> bookingPast = bookingRepository.findAllByOwnerIdAndStartAfter(1L, LocalDateTime.now(), Pageable.unpaged());
        assertEquals(bookingPast.size(),2);
        assertTrue(bookingPast.get(0).getEnd().isAfter(LocalDateTime.now()));

        List<Booking> bookingRejected = bookingRepository.findAllByBookerIdAndStatus(1L,BookingStatus.REJECTED,Pageable.unpaged());
        assertEquals(bookingPast.size(),2);
    }
